...
/// The ordinary arms use the same row count so results are comparable across shapes.
const ROWS: usize = 1 << 14;

/// The overlapping-polygon arm pays for topology graphs on every row. Keep its CodSpeed CPU
/// simulation workload bounded without shrinking the rest of the production baselines.
const OVERLAPPING_POLYGON_ROWS: usize = 1 << 10;
...
mod contains {
    ...
    /// Constant container against overlapping polygons. Each row reaches the full topology path,
    /// so this arm uses a smaller fixture while the ordinary geo baselines remain at [`ROWS`].
    #[divan::bench]
