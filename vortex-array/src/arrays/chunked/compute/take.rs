...
fn take_chunked(
    ...
    // Sort (value, orig_pos) pairs so indices for the same chunk are contiguous. Null indices
    // are skipped: their final_take slots stay 0 and are masked null below.
    ...
    // Walk sorted pairs against chunk boundaries: build per-chunk filter masks, dedup repeated
    // values inline, and scatter final_take[orig_pos] = dedup_idx.
    ...
    for chunk_idx in 0..nchunks {
        ...
        if !chunk_pairs.is_empty() {
            let mut local_indices: Vec<usize> = Vec::with_capacity(chunk_pairs.len());
            ...
            flattener.push(array.chunk(chunk_idx).filter(filter_mask)?, ctx)?;
        }

        cursor = range_end;
    }
    if cursor != pairs.len() {
        vortex_bail!(OutOfBounds: usize::try_from(pairs[cursor].0)?, 0, array.as_ref().len());
    }
