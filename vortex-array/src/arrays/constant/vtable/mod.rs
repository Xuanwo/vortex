...
impl VTable for Constant {
    ...
    fn append_to_builder(
        ...
        match array.dtype() {
            ...
            // A constant struct is a constant array per field, so each field's builder takes one
            // and stays constant-encoded. Canonicalizing first would build the same per-field
            // constants only to hand them straight back.
            ...
            // An extension array is its storage wearing a dtype, so a constant one is a constant
            // storage array.
            DType::Extension(..) => match builder.as_any_mut().downcast_mut::<ExtensionBuilder>() {
                Some(b) => b.append_constant(scalar.as_extension(), n, ctx)?,
                None => append_via_canonical(array, builder, ctx)?,
            },
            // A fixed-size list cannot share one copy of its elements between rows, but it can at
            // least avoid materializing the tiled elements as an array before copying them in.
            DType::FixedSizeList(..) => {
